...
const SYNTHETIC_STALK_RESPONSE = 18;
const SYNTHETIC_IDLE_SAG = 0.28;
const SYNTHETIC_HELD_SAG = 0.08;
const DROP_SHADOW_SURFACE_OFFSET = 0.035;
...
function isSyntheticStalkHeld(controlMode, side) {
  return controlMode === 'both' || controlMode === side;
}

function getWorldRight(rotationY) {
  return new THREE.Vector3(Math.cos(rotationY), 0, -Math.sin(rotationY));
}

export class SnailActor {
  declare bodyPosition: any;
  declare bodyRadius: any;
  declare bodyVelocity: any;
  declare controlIntensity: any;
  declare groundHeight: any;
  declare impactMomentumFactor: any;
  declare impactPower: any;
  declare impactThreshold: any;
  declare stalkSegmentRadius: any;
  declare terrainConfig: any;
  declare arenaRadius: any;
  declare body: any;
  declare bodyCenter: any;
  declare controlMode: any;
  declare damageColor: any;
  declare damageFlashDuration: any;
  declare damageFlashTime: any;
  declare deathBurst: any;
  declare deathBurstDuration: any;
  declare deathBurstEnabled: any;
  declare deathBurstGravity: any;
  declare deathBurstPieces: any;
  declare dropShadow: any;
  declare dropShadowMaterial: any;
  declare eyeTipPosition: any;
  declare eyeTipVelocity: any;
  declare hasReceivedMatchState: any;
  declare health: any;
  declare isDamaged: any;
  declare lastMotionDelta: any;
  declare maxHealth: any;
  declare mesh: any;
  declare originalBodyColor: any;
  declare previousEyeTipPosition: any;
  declare shell: any;
  declare shellCriticalColor: any;
  declare shellDamagedColor: any;
  declare shellHealthyColor: any;
  declare spawnDropHeight: any;
  declare speed: any;
  declare stalkConstraintIterations: any;
  declare stalkDamping: any;
  declare stalkDrivePull: any;
  declare stalkGravity: any;
  declare stalkIdlePull: any;
  declare stalkLength: any;
  declare stalkNeutralPose: any;
  declare stalkPitchMax: any;
  ...
  constructor(config) {
    ...
    this.dropShadow = this.createDropShadow();
    this.dropShadowMaterial = this.dropShadow.material;
    this.mesh.add(this.dropShadow);
  ...
  createDropShadow() {
    ...
    shadow.scale.set(this.bodyRadius * 1.15, this.bodyRadius * 0.72, 1);
  ...
  updateShared(delta) {
    ...
    this.updateDropShadow({
      ...
      connected: true,
      grounded: (this as any).isGrounded ?? true,
      health: this.health,
      supportKind: 'terrain'
    });
  ...
  applyMatchState(state, delta = 0) {
    ...
    this.updateDropShadow(state);

    const hasAuthoritativeStalks = STALK_SIDE_KEYS.some((side) => Boolean(state.stalks?.[side]?.nodes));
    if (hasAuthoritativeStalks) {
  ...
  updateDropShadow(state) {
    if (!this.dropShadow) {
      return;
    }

    const rawTerrainHeight = getTerrainHeight(this.mesh.position.x, this.mesh.position.z, this.terrainConfig);
    const bodyRadius = Number.isFinite(state.bodyRadius) ? state.bodyRadius : this.bodyRadius;
    const bodyFootHeight = this.mesh.position.y - bodyRadius;
    const groundedOnProp = state.grounded && state.supportKind === 'prop';
    const shadowWorldY = (groundedOnProp ? Math.max(rawTerrainHeight, bodyFootHeight) : rawTerrainHeight) +
      DROP_SHADOW_SURFACE_OFFSET;
    const dropDistance = Math.max(0, this.mesh.position.y - shadowWorldY - bodyRadius);
    const scale = THREE.MathUtils.clamp(1 + dropDistance * 0.035, 0.65, 1.85);
    const opacity = THREE.MathUtils.clamp(0.3 - dropDistance * 0.012, 0.055, 0.3);

    this.dropShadow.position.set(0, shadowWorldY - this.mesh.position.y, 0);
    this.dropShadow.scale.set(this.bodyRadius * 1.15 * scale, this.bodyRadius * 0.72 * scale, 1);
    this.dropShadowMaterial.opacity = opacity;
    this.dropShadow.visible = this.mesh.visible && state.connected !== false && state.health > 0;
  }
  ...
  getSyntheticStalkPose(side, state) {
    ...
    const held = isSyntheticStalkHeld(controlMode, side);
    const yaw = this.stalkNeutralPose.yaw + (held ? sideSign * (0.2 + intensity * 0.62) : sideSign * 0.08);
    const pitch = THREE.MathUtils.clamp(
      this.stalkNeutralPose.pitch + (held ? -0.08 - intensity * 0.08 : 0.02),
      this.stalkPitchMin,
      this.stalkPitchMax
    );
    const reach = held ? 0.92 + intensity * 0.28 : 0.82;

    return { yaw, pitch, reach, held, intensity };
  }
  ...
  createSyntheticStalkNodes(stalk, pose) {
    ...
    const sway = pose.held ? sideSign * pose.intensity * 0.16 : sideSign * 0.04;
    const nodes: THREE.Vector3[] = [];

    for (let index = 0; index <= this.stalkSegmentCount; index += 1) {
      const alpha = index / this.stalkSegmentCount;
      const bend = Math.sin(alpha * Math.PI);
      const node = rootWorld.clone().lerp(goalWorld, alpha);
      node.y -= bend * sag;
      node.addScaledVector(right, bend * sway);
      nodes.push(node);
    }
  ...
  applySyntheticStalkState(state, delta = 0) {
    for (const side of STALK_SIDE_KEYS) {
      const stalk = this.stalks[side];
      const pose = this.getSyntheticStalkPose(side, state);
      const desiredNodes = this.createSyntheticStalkNodes(stalk, pose);
      const previousNodes = stalk.nodes.length > 0
        ? stalk.nodes.map((node) => node.clone())
        : desiredNodes;
      const alpha = delta > 0 ? Math.min(1, SYNTHETIC_STALK_RESPONSE * delta) : 1;

      copyNodesInto(stalk.previousNodes, previousNodes);
      if (stalk.nodes.length !== desiredNodes.length) {
        copyNodesInto(stalk.nodes, desiredNodes);
      } else {
        for (let index = 0; index < desiredNodes.length; index += 1) {
          stalk.nodes[index].lerp(desiredNodes[index], alpha);
        }
      }
  ...
  getEyeStalkPosition(side: any = null) {
    if (side) {
  ...
  getEyeStalkVelocity(side: any = null) {
    if (side) {
