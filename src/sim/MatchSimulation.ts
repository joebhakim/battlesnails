...
export type StalkAuthorityMode = 'rope' | 'analytic' | 'human_rope' | 'rope_lite_dftl';
...
export function normalizeStalkAuthorityMode(value: any = null): StalkAuthorityMode {
  ...
  if (
    normalized === 'rope_lite_dftl' ||
    normalized === 'ropelite_dftl' ||
    normalized === 'dftl' ||
    normalized === 'ftl'
  ) {
    return 'rope_lite_dftl';
  }
...
function isBotLiteStalkAuthority(mode: StalkAuthorityMode, player: any) {
  return mode === 'rope_lite_dftl' && player?.profileName === 'bot';
}
...
export class MatchSimulation {
  ...
  updateStalkRopes(player, delta, bodyObstacles = [], options: any = {}) {
    ...
    const useDftlLite = isBotLiteStalkAuthority(this.stalkAuthorityMode, player);
      ...
      this.timeDetailedProfileBucket('stalkRopeSim', () => {
        const commonOptions = {
          ...
          collision: {
            ...
            includeSegmentMidpoints: !useDftlLite && fullFidelity,
        ...
        if (useDftlLite) {
          simulateStalkRopeDftl({
            ...
            bendResistance: stalk.held ? 0.34 : 0.5,
            velocityCorrection: stalk.held ? 0.72 : 0.88
          });
          return;
        }
