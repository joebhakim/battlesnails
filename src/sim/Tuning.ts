...
export interface TuningConfig {
  ...
  botMaxHealth: number;
  freeMoveSpeed: number;
  lockedMoveSpeed: number;
  botMoveSpeed: number;
  jumpVelocity: number;
  bodyGravity: number;
  bodyVerticalDamping: number;
  turnSpeed: number;
  bodyRadius: number;
  aboveGroundHeight: number;
  spawnDropHeight: number;
  trailSpeedMultiplier: number;
  trailCellSize: number;
  trailContactRadius: number;
  impactThreshold: number;
  impactMomentumFactor: number;
  stalkControlMode: StalkControlMode;
  stalkTurgidity: number;
  stalkReachSensitivity: number;
  stalkReachMin: number;
  stalkReachMax: number;
  stalkMass: number;
  stalkSegmentCount: number;
  stalkTotalLength: number;
  stalkSegmentRadius: number;
  stalkGravity: number;
  stalkDamping: number;
  stalkConstraintIterations: number;
  stalkDrivePull: number;
  stalkIdlePull: number;
  stalkTargetApproachSpeed: number;
  stalkNeutralYaw: number;
  stalkNeutralPitch: number;
  stalkYawLimit: number;
  stalkPitchMin: number;
  stalkPitchMax: number;
  stalkYawSensitivity: number;
  stalkPitchSensitivity: number;
  ...
  approachMoveScale: number;
  backoffMoveScale: number;
  strafeMoveScale: number;
  strikeMoveScale: number;
  recoverMoveScale: number;
  bothAttackChance: number;
}

export type TuningKey = keyof TuningConfig;
export type TuningValue = TuningConfig[TuningKey];
...
export interface TuningSchemaEntry {
  id: TuningKey;
  label: string;
  section: string;
  ...
  structural?: boolean;
  kind?: 'choice' | 'int';
  options?: readonly (TuningOption | TerrainPresetOption)[];
  visibleWhen?: {
    id: TuningKey;
    equals: TuningValue;
  };
...
export interface SimulationProfiles {
  human: SimulationProfile;
  bot: SimulationProfile;
  [profileName: string]: SimulationProfile;
}
...
const STALK_CONTROL_OPTIONS: readonly TuningOption[] = Object.freeze([
...
const RAW_TUNING_SCHEMA: readonly TuningSchemaEntry[] = Object.freeze([
  {
    id: 'terrainPreset',
    label: 'Preset',
  ...
  {
    id: 'bodyVerticalDamping',
    label: 'Body Y Damping',
    section: 'Movement',
    min: 0,
    max: 4,
    step: 0.05,
...
export const TUNING_SCHEMA: readonly Readonly<TuningSchemaEntry>[] = RAW_TUNING_SCHEMA.map((entry) => Object.freeze({ ...entry }));
...
function normalizeChoiceValue(entry: Readonly<TuningSchemaEntry>, rawValue: unknown): TuningValue {
  const options = new Set<any>((entry.options ?? []).map((option) => option.value));
  return typeof rawValue === 'string' && options.has(rawValue) ? rawValue as TuningValue : entry.defaultValue;
}

function normalizeEntryValue(entry: Readonly<TuningSchemaEntry>, rawValue: unknown): TuningValue {
  if (entry.kind === 'choice') {
    return normalizeChoiceValue(entry, rawValue);
  }

  const fallback = Number(entry.defaultValue);
  const numericValue = typeof rawValue === 'number' && Number.isFinite(rawValue) ? rawValue : Number(rawValue);
  const clamped = clamp(
    Number.isFinite(numericValue) ? numericValue : fallback,
    entry.min ?? Number.NEGATIVE_INFINITY,
    entry.max ?? Number.POSITIVE_INFINITY
  );
  const stepped = roundToStep(clamped, entry.step ?? 1);
  return entry.kind === 'int' ? Math.round(stepped) : stepped;
}

export const DEFAULT_TUNING_CONFIG: Readonly<TuningConfig> = Object.freeze(
  Object.fromEntries(
    TUNING_SCHEMA.map((entry) => [entry.id, normalizeEntryValue(entry, entry.defaultValue)])
  ) as unknown as TuningConfig
);
...
export function normalizeTuningConfig(rawConfig: Partial<TuningConfig> = {}): TuningConfig {
  const normalized = {} as TuningConfig;
  ...
  for (const entry of TUNING_SCHEMA) {
    (normalized as any)[entry.id] = normalizeEntryValue(entry, configWithMigrations[entry.id]);
  }
...
export function normalizeDuelTuningConfig(rawConfig: Partial<TuningConfig> = {}): TuningConfig {
  return {
...
export function hasStructuralTuningChanges(previousConfig: Partial<TuningConfig>, nextConfig: Partial<TuningConfig>): boolean {
...
export function createTerrainConfigFromTuning(config: Partial<TuningConfig> = DEFAULT_TUNING_CONFIG): TerrainConfig {
...
export function isTuningEntryVisible(entry: Readonly<TuningSchemaEntry>, values: Partial<TuningConfig> = DEFAULT_TUNING_CONFIG): boolean {
...
export function createSimulationProfiles(config: Partial<TuningConfig> = DEFAULT_TUNING_CONFIG): SimulationProfiles {
  const tuning = normalizeTuningConfig(config);
  const sharedProfile = {
    ...
    verticalDamping: tuning.bodyVerticalDamping,
...
export function createBotControllerConfig(config: Partial<TuningConfig> = DEFAULT_TUNING_CONFIG): BotControllerConfig {
  const tuning = normalizeTuningConfig(config);
  return {
...
export function formatTuningValue(entry: Readonly<TuningSchemaEntry>, value: TuningValue): string {
  if (entry.kind === 'choice') {
