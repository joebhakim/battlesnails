...
export class UI {
  declare appliedSimulatorTuningValues: any;
  declare instructions: any;
  declare pendingSimulatorTuningValues: any;
  declare rightStalkIndicator: any;
  declare simulatorCopyStatus: any;
  declare simulatorPanel: any;
  declare simulatorTuningControlRefs: any;
  declare simulatorTuningControlSchema: any;
  declare simulatorTuningControls: any;
  ...
  declare simulatorRestartVisualButton: any;
  declare simulatorRunButton: any;
  declare simulatorSeedInput: any;
  declare simulatorStatus: any;
  declare simulatorTuningApplyButton: any;
  declare simulatorTuningDefaultsButton: any;
  ...
  declare startMenuCopy: any;
  declare startMenuTitle: any;
  declare startMultiplayerButton: any;
  declare startSimulatorButton: any;
  declare startSinglePlayerButton: any;
  declare startTestModeButton: any;
  declare testApplyButton: any;
  declare testControlRefs: any;
  declare testControlSchema: any;
  declare testControls: any;
  declare testPanel: any;
  declare testPanelCopy: any;
  declare testPanelKicker: any;
  declare testPanelTitle: any;
  declare testResetArenaButton: any;
  ...
  updatePerformanceStats({ fps = null, tps = null }: any = {}) {
    if (this.fpsValue) {
      this.fpsValue.textContent = Number.isFinite(fps) ? `${Math.round(fps)}` : '--';
    }

    if (this.tpsValue) {
      this.tpsValue.textContent = Number.isFinite(tps) ? `${Math.round(tps)}` : '--';
    }
  }
  ...
  renderSinglePlayerSetupControls(schema, values) {
    ...
    for (const entry of schema) {
      ...
      select.addEventListener('change', (event) => {
        this.singlePlayerOptionValues[entry.id] = (event.currentTarget as HTMLInputElement | HTMLSelectElement).value;
      });
  ...
  showTestPanel({ schema, values, onApply, onResetArena, onResetDefaults, header = {} }: any) {
    const labels = {
      kicker: 'Test Mode',
      title: 'Snail Lab',
    ...
    for (const [section, entries] of groups.entries()) {
      ...
      for (const entry of entries) {
        ...
        input.addEventListener('input', (event) => {
          this.handleSimulatorTuningInput(entry, (event.currentTarget as HTMLInputElement | HTMLSelectElement).value, valueElement);
        });
  ...
  renderTestControls(schema, values) {
    ...
    for (const [section, entries] of groups.entries()) {
      ...
      for (const entry of entries) {
        ...
        input.addEventListener('input', (event) => {
          this.handleTestControlInput(entry, (event.currentTarget as HTMLInputElement | HTMLSelectElement).value, valueElement);
        });
        input.addEventListener('change', (event) => {
          this.handleTestControlInput(entry, (event.currentTarget as HTMLInputElement | HTMLSelectElement).value, valueElement);
        });
